...
def main():
    ...
    if measure_config['operator']['name'] == 'super_resolution':
        # print(measure_config['operator']['name']+'_'+ str(measure_config['operator']['scale_factor']))
        out_path = os.path.join(args.save_dir, task_config['conditioning']['method'],measure_config['operator']['name']+'_'+ str(measure_config['operator']['scale_factor']))
    elif measure_config['operator']['name'] == 'inp':
        out_path = os.path.join(args.save_dir, task_config['conditioning']['method'],measure_config['operator']['name']+'_'+measure_config['operator']['mask_type'])
    elif measure_config['operator']['name'] == 'deblur':
        out_path = os.path.join(args.save_dir, task_config['conditioning']['method'],measure_config['operator']['name']+'_'+measure_config['operator']['type'])
    else:
    ...
    if measure_config['operator']['name'] == 'super_resolution':
        ...
        if measure_config['operator']['name'] == 'inp_lolcat':
            loaded = np.load("inp_masks/lolcat_extra.npy")
            # plt.imsave('inp_masks/lolcat_extra.png',loaded,cmap='gray')
            mask = torch.from_numpy(loaded).to(device).reshape(-1)
            missing_r = torch.nonzero(mask == 0).long().reshape(-1) * 3
        elif measure_config['operator']['name'] == 'inp_lorem':
            loaded = np.load("inp_masks/lorem3.npy")
            # plt.imsave('inp_masks/lorem3.png',loaded,cmap='gray')
            mask = torch.from_numpy(loaded).to(device).reshape(-1)
            missing_r = torch.nonzero(mask == 0).long().reshape(-1) * 3
        else:
            # missing_r = torch.randperm(config.data.image_size**2)[:config.data.image_size**2 // 2].to(self.device).long() * 3
            # torch.save(missing_r.detach().cpu(),'inp_masks/randperm.pt')
            # missing_r = torch.load('inp_masks/randperm.pt').to(device)
            # mask_gen = mask_generator(seed=0,mask_type='random',mask_prob_range=[0.92, 0.92],image_size=256)
            mask_gen = mask_generator(seed=0,mask_type=measure_config['operator']['mask_type'],mask_prob_range=[0.92, 0.92],mask_len_range=[128,129],image_size=256)
            ref_img = torch.ones(1,3,256,256).cuda()
            loaded = mask_gen(ref_img)
            mask = loaded[0,0,...].reshape(-1)
            missing_r = torch.nonzero(mask == 0).long().reshape(-1) * 3
    ...
    for ref_img, filenames in pbar:
        ...
        sample = sample_fn(x_start=x_start, measurement=y_n, H_funcs=H_funcs, noise_std = noiser.sigma, record=False, save_root=out_path)
        ...
        if measure_config['operator']['name']  == 'color':
            ...
            y_n = H_funcs.H_pinv(y_n).view(y_n.shape[0], model.in_channels, model.image_size, model.image_size)
            y_n += H_funcs.H_pinv(H_funcs.H(torch.ones_like(y_n))).reshape(*y_n.shape) - 1
        else:
            input_size = int(model.image_size/ratio)  
            y_n = y_n.reshape(-1,model.in_channels,input_size,input_size)
