    ...
    def p_sample_loop(self,
        ...
        for idx in pbar:
            ...
            img = measurement_cond_fn(x_t=out['sample'],
                                      ...
                                      pseudonoise_scale = scale_pseudo_noise[time], scale_coef = self.mean_processor.out_coef,idx=idx)
            img = img.detach_()
            self.mean_processor.out_coef = 1.0           
...
@register_sampler(name='ddim_ori')
class DDIM(SpacedDiffusion):
    def p_sample(self, model, x, t, eta=0.0):
        ...
        if t >=0:
            ...
            # if alpha_bar[0,0,0,0] <0.2:
            # mean_pred = (
            #     out["pred_xstart"] * torch.sqrt(alpha_bar_prev)
            #     + torch.sqrt(1 - alpha_bar_prev) * eps * torch.sqrt(alpha_bar)
            # )

            # sample = mean_pred
            # sigma = 1.0
            # if t != 0:
            #     sample += sigma * noise *torch.sqrt(1 - alpha_bar) * torch.sqrt(1 - alpha_bar_prev)
            # else:
            mean_pred = (
                out["pred_xstart"] * torch.sqrt(alpha_bar_prev)
                + torch.sqrt(1 - alpha_bar_prev) * eps * np.sqrt(0.001)
            )

            sample = mean_pred
            sigma = 1.0
            if t != 0:
                sample += sigma * noise *np.sqrt(0.999) * torch.sqrt(1 - alpha_bar_prev)
...
@register_sampler(name='ddim_ori_ori')
class DDIMORI(SpacedDiffusion):
    def p_sample(self, model, x, t, eta=0.0):
        out = self.p_mean_variance(model, x, t)
        ...
        self.ddim_coef_eps = (np.sqrt(1.0 / self.alphas_cumprod - 1) - np.sqrt(1.0 / self.alphas_cumprod_prev - 1))*np.sqrt(self.alphas_cumprod_prev)
        # self.ddim_coef = (np.sqrt(1.0 / alphas_cumprod - 1))*np.sqrt(alphas_cumprod_prev)
        self.ddim_coef_x = 1.0/np.sqrt(self.alphas)
        self.ddim_coef_noise = np.sqrt((1.0 / self.alphas_cumprod - 1) - (1.0 / self.alphas_cumprod_prev - 1))*np.sqrt(self.alphas_cumprod)
        ...
        ddim_coef_eps = extract_and_expand(self.ddim_coef_eps, t, x)
        ddim_coef_x = extract_and_expand(self.ddim_coef_x, t, x)
        ddim_coef_noise = extract_and_expand(self.ddim_coef_noise, t, x)
        sigma = 0.0 #ddim_coef_noise
        noise = torch.randn_like(x)
        mean_pred = (
            x * ddim_coef_x
            - ddim_coef_eps * eps #*2.0
        )
